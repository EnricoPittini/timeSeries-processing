...
The timeseries are daily time series, i.e. the dates are always days. (Dates and days are used as synonyms).
...
def find_k_years_ago_days(day, k=1, n_days=11):
    """
    Return, given a day, the days which are centered in that day but k years ago.
    ...
    ----------
    ValueError
        When `n_days` it's neither an odd positive integer nor "month" nor "season".
    """
    year = day.year
    month = day.month
    d = day.day
...
def find_current_year_days(day, n_days=11, current_day=False):
    """
    Return, given a day, the preceding days of the same year which are centered in that day.
    ...
    ----------
    ValueError
        When `n_days` it's neither an integer nor "month" nor "season".
    """
...
def group_days_by(days, criterion):
    """
    Group the given vetor of days according to the given criterion.
    ...
    ----------
    ValueError
        When `criterion` it's neither an "year" nor "month" nor "season".

    Notes
    ----------
    For the sake of completeness, it's important to say that if `critetion` is either "month" or "season", are grouped
    togheter also days of different years.
    """
...
def plot_timeSeries(df, col_name, divide=None, xlabel="Days", line=True, title="Time series values", figsize=(9,9)):
    ...
    ----------
    df: pd.DataFrame
        DataFrame indexed by days (i.e. pd.DatetimeIndex).
    col_name: str
        Indicates the specified column to plot.
    divide: str
        Indicates if and how divide the values plotted.
    ...
    line: bool
        Indicates wheter to connect the points with a line.
    title: str
        Title of the plot.
    figsize: tuple
...
def add_timeSeries_dataframe(df, df_other, y_col=None, scale_y=True):
    ...
    The two DataFrames are concatenated in a new DataFrame, i.e. the resulting DataFrame contains all the columns in `df` and
    `df_other`.
    This concatenation is done with respect to the former: this means that all the days of `df` are kept, while only the days
    of `df_other` that are also in `df` are kept.

    In addition, the resulting DataFrame is authomatically split into the X matrix and the y vector, which are respectively
    the matrix containing the explanatory features and the vector containing the response feature.
    (The response feature is the one target of prediction analysis tasks).
    ...
    df_other: pd.DataFrame
        Other DataFrame indexed by days (i.e. pd.DatetimeIndex), to add to the former.
    y_col: str
        Indicates which is the column of the resulting DataFrame to use as y column.
    scale_y: bool
        Indicates wheter scale or not the values of the response feature y.
    ...
    X: np.array
        2-dimensional numpy array which contains the explanatory features.
    y: np.array
        1-dimensional numpy array which contains the response features.
...
def add_k_previous_days(df, col_name, k, y_col=None, scale_y=True):
    ...
    It's built a new DataFrame, which is created from `df` adding `k` new columns. These `k` new columns contain the values
    of the column `col_name` but with regard to, respectively: the day before; the 2-day before; ... ; the k-day before.
    In this way in the resulting DataFrame fro each day there are informations about the feature `col_name` up to `k` days
    before.
    These `k` columns are, respectively, called: "col_name_1", "col_name_2", ..., "col_name_k".

    The first `k` days are removed from the resulting DataFrame: that is beacuse for the first `k` days there isn't enough
    ...
    df: pd.DataFrame
        DataFrame indexed by days (i.e. pd.DatetimeIndex).
    col_name: str
        Indicates which is the column from which build the `k` new columns.
    k: int
        Indicates how many previous days are to take into account (i.e. how many new columns are built).
    y_col: str
        Indicates which is the column of the resulting DataFrame to use as y column.
    scale_y: bool
        Indicates wheter scale or not the values of the response feature y.
    ...
    ----------
    ValueError
        When `df` does not contain a contigous sequence of days (i.e. there are missing days in `df`).
...
def add_k_years_ago_statistics(df, df_k_years_ago, k=1, days_to_select=11, stat="mean", columns_to_select=None,
    ...
    Let 'm' be the number of selected columns of `df_k_years_ago` (by default all the columns, see the `columns_to_select`
    parameter).
    It's built a new DataFrame, which is created from `df` adding `m` new columns. (The resulting DataFrame has the same
    index of `df`). These `m` new columns contain the values computed from the associated columns of `df_k_years_ago`
    ...
    Once are selected the `k` years ago days in `df_k_years_ago`, is computed an unique value for the new column 'new_column'
    and for the day 'day' applying a certain statistical aggregation (specified by the input parameter `stat`) on the values
    of these selected days in the column of `df_k_years_ago` associated to 'new_column'.

    `days_to_select` specify which `k` years ago days in `df_k_years_ago` are selected for each 'day' of `df`. The semantic
    is quite similar to the parameter `n_days` of the find_k_years_ago_days function (it can be either an odd integer or
    "month" or "season").

    Actually, `days_to_select` can also be more powerful. `days_to_select` can be a predicate (i.e function that returns a
    ...
    where
        ...
        - `df_k_years_ago` it's the other given DataFrame, containing `k` years ago days.
    The function returns True if and only if `k_years_ago_day` is a day that has to be selected for `day`.

    For a certain 'day' of `df` can happend that it is not selected any `k` years ago day. Which means that this 'day' has a
    missing value for each of the 'm' new columns (i.e. 'm' missing values).
    In this case, if `replace_miss` it's True, all the missing values are filled: the missing value for the new column
    'new_column' it's filled computing the mean of all the values in the associated column in `df_k_years_ago`.
    Otherwise, if `replace_miss` it's False, the 'm' missing values are simply kept as Nan.

    So, in the end, are created 'm' new columns in the resulting DataFrame, from the selected 'm' columns of
    `df_k_years_ago`.
    From the selected column with name "col" of `df_k_years_ago`, it's created the correspondent column "k_years_ago_col" in
    the resulting DataFrame.
    ...
    df_k_years_ago: pd.DataFrame
        DataFrame indexed by days (i.e. pd.DatetimeIndex). It should contain `k` years ago days with respect to the days in
        `df`.
    k: int
        Indicates which previous year, with respect to the days in `df`, is to be taken into account (i.e. `k` years ago).
        It must be a positive integer.
    days_to_select: int or str or callable
        Indicates, for each day of `df`, which are the `k` years ago days in `df_k_years_ago` that are selected.
    ...
    replace_miss: bool
        Indicates wheter fill the missing values or keep them as Nan.
        (The missing values are generated for each day of `df` for which is not selected any `k` years ago day in
        `df_k_years_ago`).
    y_col: str
        Indicates which is the column of the resulting DataFrame to use as y column.
    scale_y: bool
    ...
    ValueError
        ...
        - When `stat` is not neither "mean" nor "min" nor "max".

    Warns
    ----------
    UserWarning
        - When, for a day of `df`, is not selected any `k` years ago days in `df_k_years_ago`.
        - When, for a day of `df`, are found less `k` years ago days compared to the ones expected.
    ...
    Notes
    ----------
    If add_k_years_ago_statistics it's applied multiple times with the same `k` on the same `df` and `df_k_years_ago`,
    potentially are created columns with the same name.
    For instance, if add_k_years_ago_statistics it's applied three times with the same `k` on the same DataFrames, from the
    `df_k_years_ago` column "col" are potentially created three different columns with the same name "k_years_ago_col".
    ...
    # 2 dimensional list (matrix-like). It contains, for each day of `df`, the values computed on `df_k_years_ago` for that
    ...
    for day in df.index: # Iterate through all the days in `df`
         ...
         if not callable(days_to_select): # days_to_select it's not a function : either an odd integer or "month" or "season"
         ...
         if len(k_years_ago_days)==0: # There isn't any selected `k` years ago day
             warnings.warn("No "+str(k)+" years ago days have been found for the day " + day.strftime('%Y-%m-%d'))
         # They have been selected less `k` years ago days than the ones expected
         ...
         # There isn't any selected `k` years ago day and `replace_miss` is True: the missing values are filles with the
...
def add_current_year_statistics(df, df_current_year, days_to_select=11, current_day=False, stat="mean",
    ...
    Let 'm' be the number of selected columns of `df_current_year` (by default all the columns, see the `columns_to_select`
    parameter).
    It's built a new DataFrame, which is created from `df` adding `m` new columns. (The resulting DataFrame has the same
    index of `df`). These `m` new columns contain the values computed from the associated columns of `df_current_year`
    considering the preceding days of the same year of the ones in `df`.
    ...
    Once are selected the preceding days of the same year from `df_current_year`, is computed an unique value for the new
    column 'new_column' and for the day 'day' applying a certain statistical aggregation (specified by the input parameter
    `stat`) on the values of these selected days in the column of `df_current_year` associated to 'new_column'.

    `days_to_select` specify which preceding same year days are selected from `df_current_year` for each 'day' of `df`. The
    semantic is quite similar to the parameter `n_days` of the find_current_year_days function (it can be either an integer
    or "month" or "season").
    ...
    where
        ...
        - `df_current_year` it's the other given DataFrame, containing same year days.
    The function returns True if and only if `current_year_day` is a day that has to be selected for `day`.

    For a certain 'day' of `df` can happend that it is not selected any preceding day of the same year. Which means that this
    ...
    Otherwise, if `replace_miss` it's False, the 'm' missing values are simply kept as Nan. (It's not removed any day).

    If `current_day` is True, each 'day' of `df` is itself a potential same year day that can be selected. I.e. are not
    considered only the preceding days of the same year.
    (This is applied also in the selection of the days to use to fill the missing values).

    So, in the end, are created 'm' new columns in the resulting DataFrame, from the selected 'm' columns of
    `df_current_year`.
    From the selected column with name "col" of `df_current_year`, it's created the correspondent column "current_year_col"
    ...
    df_current_year: pd.DataFrame
        DataFrame indexed by days (i.e. pd.DatetimeIndex). It should contain same year days with respect to the days in
        `df`.
    days_to_select: int or str or callable
        Indicates, for each day of `df`, which are the preceding days of the same year in `df_current_year` that are
        selected.
        It must be either an integer or "month" or "season" or a predicate (i.e. a function that return a boolean).
        The function signature must be
                (day: pd.TimeStamp, df: pd.DataFrame, current_year_day: pd.TimeStamp, df_current_year: pd.DataFrame): bool
    current_day: bool
        Indicates if, each day of `df`, can be potentially selected as a day of the same year for itself.
    stat: str
    ...
    replace_miss: bool
        Indicates wheter fill the missing values or keep them as Nan.
        (The missing values are generated for each day of `df` for which is not selected any same year day in
        `df_current_year`).
    y_col: str
        Indicates which is the column of the resulting DataFrame to use as y column.
    scale_y: bool
    ...
    ----------
    UserWarning
        - When, for a day of `df`, is not selected any preceding day of the same year from `df_current_year`.
        - When, for a day of `df`, are found less preceding days of the same year compared to the ones expected.
    ...
    Notes
    ----------
    If add_current_year_statistics it's applied multiple times with on the same `df` and `df_current_year`,
    potentially are created columns with the same name.
    For instance, if add_current_year_statistics it's applied three times on the same DataFrames, from the `df_current_year`
    ...
    # 2 dimensional list (matrix-like). It contains, for each day of `df`, the values computed on `df_current_year` for that
    # day (i.e. the values to be added in the resulting DataFrame).
    # So, it's a matrix that contains as many rows as the days in `df` and as many columns as the selected columns in
    # `df_current_year`.
    statistics_list = []
    ...
    # These are the days for which is not select any same year day and for which there aren't preceding days in
    # `df_current_year` (preceding days or also the day itself, if `current_day` is True).
    days_to_remove = []

    for day in df.index: # Iterate through all the days in `df`

         if not callable(days_to_select): # days_to_select it's not a function : either an integer or "month" or "season"
         ...
         if len(current_year_days)==0: # There isn't any selected same year day
             warnings.warn("No current year days have been found for the day " + day.strftime('%Y-%m-%d'))
        # They have been selected less same year days than the ones expected
        ...
        # There isn't any selected same year day and `replace_miss` is True: the missing values are filles with the
        # means of the columns.
        # (If `replace_miss` is False the missing values are kept as Nan)
...
def add_upTo_k_years_ago_statistics(df, df_upTo_k_years_ago, k=1, current_year=True, days_to_select=11, current_day=False,
    ...
    Let 'm' be the number of selected columns of `df_upTo_k_years_ago` (by default all the columns, see the
    `columns_to_select`  parameter).
    It's built a new DataFrame, which is created from `df` adding `m` new columns. (The resulting DataFrame has the same
    index of `df`). These `m` new columns contain the values computed from the associated columns of `df_upTo_k_years_ago`
    ...
    Let's see that more in depth. For each integer 'i' from 1 to `k`, are selected the 'i' years ago days centered in 'day'
    and contained in `df_upTo_k_years_ago` (see the find_k_years_ago_days function): from these selected 'i' years ago days,
    is computed an unique value applying a certain statistical aggregation (specified by the input parameter `stat`) on the
    values of these selected days in the column of `df_k_years_ago` associated to 'new_column'.
    In this way, for 'day' and 'new_column' are computed `k` values, for each integer 'i' from 1 to `k`. In the end, is
    ...
    On the whole, is computed an aggregation with 2 levels on the days of up to `k` years ago with respect to 'day'.
        - Is computed an aggregation on the selected days of 'i' years ago days, for i between 1 and `k`.
        - Is computed an aggregation on the `k` values computed for each of the previous years, up to `k` years ago.

    Basically, this is implemented applying `k` times the function add_k_years_ago_statistics: for each 'i' between 1 and
    `k`, is applied  add_k_years_ago_statistics with his input parameter 'k' equal to 'i'.
    It's applied add_k_years_ago_statistics on all the previous years, up to `k` years ago.
    ...
    The meaning of the input parameters `days_to_select`, `stat`, `replace_miss`, ..., are the same of the ones in
    add_k_years_ago_statistics.

    In particular, `days_to_select` specify which 'i' years ago days in `df_upTo_k_years_ago` are selected for each 'day' of
    `df`, for 'i' from 1 to `k`. It can be either an odd integer or "month" or "season" or a predicate (i.e. function that
    returns a bool).
    The signature of the function must be:
        (day: pd.TimeStamp, df: pd.DataFrame, i_years_ago_day: pd.TimeStamp, df_k_years_ago: pd.DataFrame): bool.

    If `current_year` is True, is taken into account also the current year, and not only the preceding years up to `k` years
    ago.
    This means that, for each 'day' of `df`, are computed `k`+1 values: from the current year; from the previous year; ...;
    ...
    (If `current_year` is True, is applied add_current_year_statistics on time, and then is applied
    add_k_years_ago_statistics `k` times).

    So, in the end, are created 'm' new columns in the resulting DataFrame, from the selected 'm' columns of
    `df_upTo_k_years_ago`.
    From the selected column with name "col" of `df_upTo_k_years_ago`, it's created the correspondent column
    "upTo_k_years_ago_col" in the resulting DataFrame.
    ...
    df_upTo_k_years_ago: pd.DataFrame
        DataFrame indexed by days (i.e. pd.DatetimeIndex). It should contain up to `k` years ago days with respect to the
        days in `df`.
    k: int
        Indicates how many previous years have to be taken into account (i.e. are taken into account all the previous years
        up to `k` years ago).
        It must be a positive integer.
    current_year: bool
        Indicates wheter consider also the current year or not: in the former case, are taken into account all the years from
        the current up to `k` years ago.
    days_to_select: int or str or callable
        Indicates, for each day of `df`, which are the days in `df_upTo_k_years_ago` that have to be selected, for each year
        from the previous up to `k` years ago.
        It must be either an odd integer or "month" or "season" or a predicate (i.e. a function that return a boolean).
    ...
    stat: str
        Indicates the statistical aggregation to perform. It can be either "mean" or "min" or "max".
        This is applied in 2 levels: both for each previous year (up to `k` years ago) and for the aggregation of the `k`
        computed values (`k`+1 if `current_year` is True).
    columns_to_select: list
        List of strings which indicates the columns of `df_upTo_k_years_ago` that have to be taken into account.
        If it's None, all the columns of `df_upTo_k_years_ago` are considered.
    replace_miss: bool
        Indicates wheter fill the missing values.
    y_col: str
        Indicates which is the column of the resulting DataFrame to use as y column.
    scale_y: bool
        Indicates wheter scale or not the values of the response feature y.
    ...
    Notes
    ----------
    - If add_upTo_k_years_ago_statistics it's applied multiple times with the same `k` on the same `df` and
      `df_upTo_k_years_ago`, potentially are created columns with the same name.
      For instance, if add_upTo_k_years_ago_statistics it's applied three times with the same `k` on the same DataFrames,
      from the `df_upTo_k_years_ago` column "col" are potentially created three different columns with the same name
    ...
    - The meaning of `replace_miss` is the same of add_k_years_ago_statistics. For each previous year up to `k` years ago, if
      is not found any selected day and `replace_miss` is True, is computed the mean of the whole `df_upTo_k_years_ago`
      DataFrame: this is the value calculated for that year (value that will be aggregated with the other `k`).
    ...
    # I.e. the first DataFrame is relating to the current year, the second to the previous year, the third to two years ago,
    # ... .
    ...
    # In order to create "upTo_k_years_ago_col", are taken all the `k`+1 columns in 'df_supp' related to "col" (
    # "current_year_col", "1_years_ago_col", "2_years_ago_col", ... , "k_years_ago_col") and is computed a certain
    # aggregation "mean","min","max").
    new_columns = []
    for col in columns_to_select:
        associated_columns = [str(i)+"_years_ago_"+col for i in range(1,k+1)]
        if current_year: # The column "current_year_col" is to be considered only if `current_year` is True
